...
def get_secondary_connections(network, user):
    ...
    for connection in get_connections(network, user):
        secondary += get_connections(network, connection)
...
def find_game_party(network, user, game):
    party = []
    if user not in network:
        return None
    if game not in get_games_liked(network, user):
        return None
    connections = get_secondary_connections(network, user) + get_connections(network, user)
    connections = list(set(connections))
    for connection in connections:
        if game in get_games_liked(network, connection) and connection != user:
            party.append(connection)
    return party    
